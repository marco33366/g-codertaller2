...
G-Coder integra verticalmente el flujo de Trujillo 3D:

- Operativo: el `operario` usa el conversor para analizar STL y generar G-code.
- Supervision: el `jefe_operarios` gestiona usuarios operarios desde `/users`.
- Gerencial: el `gerente` revisa `/dashboard` y `/logs` para supervisar indicadores y auditoria.

La integracion horizontal se refleja en autenticacion, gestion de usuarios, conversion CNC, seguridad por roles, privacidad, auditoria e indicadores gerenciales consumidos desde el backend.
...
- `gerente` / `Gerente12345`: ve `/dashboard` y `/logs`.
...
- `/`: conversor protegido.
- `/dashboard`: panel gerencial protegido para `gerente`.
...
frontend/
  ...
  src/
    features/dashboard/        # Panel gerencial e indicadores
    features/gcoder/
