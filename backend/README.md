...
G-Coder integra verticalmente los niveles operativos de Trujillo 3D:

- Operativo: el `operario` carga modelos, analiza compatibilidad y convierte STL a G-code.
- Supervision: el `jefe_operarios` administra usuarios operarios.
- Gerencial: el `gerente` consulta logs globales y el dashboard con indicadores reales.

La integracion horizontal une autenticacion, gestion de usuarios, conversion CNC, seguridad por roles, privacidad de datos sensibles, auditoria e indicadores gerenciales sobre las tablas `users` y `audit_logs`.
...
- `GET /api/dashboard/summary`: indicadores gerenciales desde `audit_logs` y `users`. Requiere rol `gerente`.
...
Consultar panel gerencial como gerente:
...
- `total_file_uploads`: conteo de `FILE_UPLOADED`.
- `total_successful_conversions`: conteo de `CONVERT_SUCCESS`.
- `total_failed_analysis`: conteo de `ANALYZE_FAILED`.
- `total_exports`: conteo de `GCODE_EXPORTED`.
- `active_users`: usuarios activos.
- `inactive_users`: usuarios inactivos.
- `last_activity_at`: fecha/hora del ultimo `audit_log`.
- `most_used_extension`: extension de archivo mas frecuente.
- `top_operator`: usuario con mas eventos `CONVERT_SUCCESS`.
